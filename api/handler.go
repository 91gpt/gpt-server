...
func GetLimitAndSkip(ctx *gin.Context) (limit, skip int, err error) {
	var (
		pageIndex int
	)
	if limit, err = strconv.Atoi(ctx.Query("page_size")); err != nil {
		return
	}
	if pageIndex, err = strconv.Atoi(ctx.Query("page_index")); err != nil {
		return
	}
